        ...
        <div className="grid grid-cols-1 md:grid-cols-[repeat(auto-fit,_minmax(350px,1fr))] mt-10 gap-5 mx-3">
            {dalok.map((dal) => { 
                return (
                    <div
                    key={dal.id}
                    className="bg-bg_secondary dark:bg-bg_secondary_dark text-font dark:text-font_dark rounded-2xl h-min">
                        ...
                        <div>
                            <p className=""
                            >{dal.Lyrics.split('$').map((versszak, id) => {
                                return <div key={id}
                                 className="m-2 my-4 text-lg md:text-xl">
                                    {
                                        versszak.split('|').map((sor, id) => {
                                        return<span key={id} className="block">{sor}</span>
                                })}</div>
                            })}</p>
                        </div>
                    </div>
